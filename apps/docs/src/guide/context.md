...
| Guarantee | Behavior |
| --- | --- |
| **Frozen** | The snapshot is frozen recursively. Handlers and the intent engine receive the exact same object; nothing can mutate it. |
| **Size limit** | Serialized to at most 8KB by default (`contextByteLimit` option). Largest keys are dropped first; a `__truncated` key lists what was removed. |
| **Failure isolation** | If the provider throws, the snapshot becomes `{ __providerError }` and the utterance still processes. If the value is not serializable, you get `{ __contextError }`. |
